...
async function createKeyRing(
  secretValue: string,
): Promise<RawAesKeyringWebCrypto> {
  const unencryptedMasterKey = fromBase64(
    await createEncryptionKeyV2(secretValue),
  );
  const wrappingSuite =
...
async function encryptSecretV2(
  plaintext: string,
...
async function decryptSecretV2(
  ciphertext: string,
  encryptionKeySecret: EncryptionKeySecret,
): Promise<string> {
  const keyRing = await createKeyRing(encryptionKeySecret.secretValue);
...
function getEncryptionKeySecret(version: string) {
  const encryptionSecrets = JSON.parse(
    import.meta.env.VITE_PUBLIC_ENCRYPTION_KEY_SECRETS!,
  ) as EncryptionKeySecret[];
  const secrets = encryptionSecrets.find(
